...
SRCFILES = \
  ...
  fltk/Flio_Inspector_Einstein.cxx \
  ...
  fltk/Fldtk_Format_Slot_Editor.cxx \
  fltk/Fldtk_Icon_Dropbox.cxx \
  ...
  fltk/Fldtk_Slot_Browser.cxx \
  fltk/Fldtk_Tmpl_Browser.cxx \
  fltk/Fldtk_Value_Slot_Editor.cxx \
  fluid/Fldtk_Inspector.cxx \
  fluid/Fldtk_Dialogs.cxx \
...
CXXFLAGS := -Wall $(shell fltk-config --use-images --cxxflags)

INCLUDES := -I. -Idtk -Ifltk -Ifluid 
...
dynetk: $(OBJFILES)
	echo Linking $@...
	$(CXX) $(DEBUGFLAGS) $(LDFLAGS) -o $@ $(OBJFILES) -lnewt -liconv $(LIBS)

.SUFFIXES: .cxx .o

.cxx.o:
	echo Compiling $<...
	$(CXX) -c $(DEBUGFLAGS) $(CXXFLAGS) $(INCLUDES) $(NEWTINCLUDES) -o $@ $<
