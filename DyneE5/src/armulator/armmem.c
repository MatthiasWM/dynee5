...
//koodailar change it for mingw 2005.12.18------------------------------------------------------
#ifdef __MINGW32__
		//this
        //		state->mem.tbp[bank] = malloc(mb[bank].len / sizeof(ARMword) * TB_INSN_LEN_MAX + mb[bank].len / TB_LEN * op_return.len);              
        //or this???		
				state->mem.tbp[bank] = NULL;
#else
