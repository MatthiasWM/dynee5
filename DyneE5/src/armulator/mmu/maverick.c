...
#include "ansidecl.h"
#include "arch/arm/common/armemu.h"
#else
#include "armdefs.h"
#include "ansidecl.h"
#include "armemu.h"
...
static ARMsdword mv_getReg64int (int);
static void mv_setRegDouble (int, double val);
static void mv_setReg64int (int, ARMsdword val);
	 ...
	 unsigned type, ARMword instr, ARMword data)
{
	...
	if (BIT (22)) {
		...
		mv_setReg64int (DEST_REG, (ARMsdword) data);
...
static void
mv_setReg64int (int regnum, ARMsdword val)
{
	reg_conv.ll = val;
