...
#ifdef WIN32
#else
ARMword max(ARMword a, ARMword b) {
  return (a>b) ? a : b;
}

ARMword min(ARMword a, ARMword b) {
  return (a<b) ? a : b;
}
#endif
 ...
 * */
int
mmu_rb_init (rb_type * rb_t, int num)
{
	int i;
	rb_entry_t *entrys;
 ...
 * */
rb_entry_t *
mmu_rb_search (rb_type * rb_t, ARMword va)
{
	int i;
...
void
mmu_rb_invalidate_entry (rb_type * rb_t, int i)
{
	rb_t->entrys[i].type = RB_INVALID;
}

void
mmu_rb_invalidate_all (rb_type * rb_t)
{
	int i;
...
void
mmu_rb_load (ARMul_State * state, rb_type * rb_t, int i_rb, int type, ARMword va)
{
	rb_entry_t *rb;
	int i;
