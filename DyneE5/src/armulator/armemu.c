...
void WriteR15 (ARMul_State *, ARMword);
...
//2003-07-11 chy: for test
		...
		if (state->NresetSig == LOW) {
			...
			state->NresetSig = HIGH; // FIXME: Matt
                        pc = state->Reg[15]; // FIXME: Matt
			break;
		}
...
//chy 2003-08-24 now #if 0 .... #endif  process cp14, cp15.reg14, I disable it...
		...
		if (temp) {
			...
			case 0xee:
				if (BIT (4)) {
                                        /* DyneE5 Native Call */
                                        if ((instr&0x0f100f10)==0x0e000210) {
                                          dnCallNative(state, instr);
                                          break;          
                                        }
					/* MCR.  */
					if (DESTReg == 15) {
